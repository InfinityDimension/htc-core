...
logfile=$root_path/logs/manager.log
ipfs_log=$root_path/logs/ipfs.log

set_network() {
  if [ "$(grep "7337a324ef27e1e234d1e9018cacff7d4f299a09c2df9be460543b8f7ef652f1" $CARITAS_CONFIG )" ];then
    NETWORK="main"
  elif [ "$(grep "cba57b868c8571599ad594c6607a77cad60cf0372ecde803004d87e679117c12" $CARITAS_CONFIG )" ];then
    NETWORK="test"
  else
    NETWORK="unknown"
  fi
...
CARITAS_CONFIG="config.json"
DB_NAME="$(grep "database" $CARITAS_CONFIG | cut -f 4 -d '"' | head -1)"
DB_UNAME="$(grep "user" $CARITAS_CONFIG | cut -f 4 -d '"' | head -1)"
DB_PASSWD="$(grep "password" $CARITAS_CONFIG | cut -f 4 -d '"' | head -1)"
...
install_prereq() {
    ...
    echo ""
    echo "-------------------------------------------------------"
    echo "caritas installer script. Version: $version"
    echo "-------------------------------------------------------"
...
install_caritas() {
    ...
    echo -n "Installing Caritas core... "
    npm install --production &>> $logfile || { echo "Could not install CARITAS, please check the log directory. Exiting." && exit 1; }
    echo -e "done.\n"
...
update_client() {
    ...
    echo -n "Updating Caritas client ... "
...
update_wallet() {
    ...
    echo -n "Updating Caritas wallet ... "
...
start_caritas() {
  ...
  start_snapshot() {
    echo -n "Starting Caritas (snapshot mode) ... "
    forever_exists=$(whereis forever | awk {'print $2'})
    if [[ ! -z $forever_exists ]]; then
        $forever_exists start -o $root_path/logs/snapshot_console.log -e $root_path/logs/snapshot_err.log app.js -c "$CARITAS_CONFIG" -s highest &>> $logfile || \
        { echo -e "\nCould not start Caritas." && exit 1; }
    fi
...
install_ipfs() {
  ...
  ipfs_exists=$(whereis ipfs | awk {'print $2'})
    if [[ ! -z $ipfs_exists ]]; then
      echo -e "IPFS is already installed. Remove it first with ./manager.bash remove_ipfs"
      exit 1;
    fi
  ...
  PORT="$(jq .port $CARITAS_CONFIG)"
  SPORT="$(jq .ssl.options.port $CARITAS_CONFIG)"
...
case $1 in
    "install")
      ...
      echo ""
      echo ""
      echo "CARITAS successfully installed"
    ...
    "status")
      if running; then
        echo "√ CARITAS is running."
        show_blockHeight
      else
        echo "X CARITAS is NOT running."
      fi
    ;;
    "start")
      parse_option $@
...
*)
    ...
    echo 'Usage: ./installer.bash install'
    exit 1
;;
esac
exit 0;
